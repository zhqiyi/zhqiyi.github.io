---
title: "安全日报 2026-07-03"
date: 2026-07-03 07:30:00 +0800
categories: 安全
tags: [安全, 网络安全, 行业, 日报]
...
> 🔥 今日封面：Claude Code被曝使用“Spyware”标记东大用户，解决方案可能是OpenAFW
...
- **[AI Security 的重点已经变了：真正让中小企业害怕的，不再是模型，而是 Agent、MCP 和 Tool](https://www.freebuf.com/news/488528.html)**
  - 未来 AI 安全越来越像传统 Web 安全。  模型只是 CPU。  真正需要防护的，是围绕模型建立起来的整个应用生态。

- **[Cursor IDE沙箱逃逸漏洞凸显提示注入可成为RCE攻击媒介](https://www.freebuf.com/articles/ai-security/488477.html)**
  - Cursor IDE沙箱逃逸漏洞通过提示注入实现RCE攻击，暴露AI编码工具重大安全隐患

- **[漏洞分析 \| LiteLLM Proxy 预认证 SQL 注入 (CVE-2026-42208)](https://www.freebuf.com/articles/web/488417.html)**
...
- **[艾默生DeltaV DCS控制器固件安全分析技术](https://www.freebuf.com/articles/ics-articles/488513.html)**
  - 本文以艾默生DeltaV DCS系统的MD Plus控制器为研究对象，从固件获取、自定义分块压缩格式解密、全局跨块解压、符号表恢复等多个角度，全方位阐述了工控安全研究中固件深度分析的思路与方法。
...
- **[前端资质越高，越来越不敢随便升级框架？](https://www.freebuf.com/news/488548.html)**
  - 在一个沉淀了无数业务逻辑的历史工程面前，请保持谨慎。

- **[原理分析 \| Agent —— Tomcat 内存马](https://www.freebuf.com/articles/web/488495.html)**
  - 检测这类内存马，核心思路是字节码比对，对关键类的运行时字节码做完整性校验，而不是只看容器层面的组件列表。

- **[原理分析 \| 反序列化内存马 —— CC2 + Tomcat三种组件 + 无文件落地](https://www.freebuf.com/articles/web/488494.html)**
  - 适合已了解 Java 反序列化基础和内存马基本原理的同学作为进阶实战参考。

- **[原理分析 \| JNDI注入内存马 —— ldap + Tomcat Filter + 无文件落地](https://www.freebuf.com/articles/web/488493.html)**
  - 文章重点拆解了三个核心问题：恶意Filter对象的字节码传递方案、StandardContext的反射获取链路、以及Filter动态注册的完整流程。

- **[Linux内核Socket Buffer本地提权漏洞复现(CVE-2026-43503) 附POC](https://www.freebuf.com/articles/vuls/488473.html)**
  - Linux kernel存在安全漏洞，该漏洞源于碎片传输辅助函数未能传播SKBFL_SHARED_FRAG位，可能导致目标skb报告skb_has_shared_frag()为假。

- **[世界杯网络安全防护实战：如何用IP数据接口识别异常访问](https://www.freebuf.com/articles/container/488471.html)**
  - IP数据接口能把IP地址转化为国家、城市、usage_type、风险评分等结构化字段，帮助赛事平台在票务、直播、支付和会员服务中更快识别异常访问，同时尽量减少对正常球迷的影响。

- **[匿名登顶中国第一，拆招“扫地僧”Agent记忆中心漏洞挖掘法](https://www.freebuf.com/articles/ai-security/488475.html)**
  - MopMonk（扫地僧）以73.1%的成功率空降CyberGym榜单第7位，刚出世即位居中国第一。

- **[财务终端已成黑客首要靶标：基于零信任的UserSafe Guard财务端防护实践](https://www.freebuf.com/articles/endpoint/488469.html)**
  - 本文结合 UserSafe Guard（USG）零信任终端防护系统，从财务岗位攻击场景出发，拆解传统防护短板，分享适配财务场景的零信任落地防御方案。

- **[2026年微软漏洞报告深度解析](https://www.freebuf.com/articles/488527.html)**
