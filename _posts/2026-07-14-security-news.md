---
title: "安全日报 2026-07-14"
date: 2026-07-14 07:30:00 +0800
categories: 安全
tags: [安全, 网络安全, 行业, 日报]
...
> 🔥 今日封面：攻击者利用疑似AI生成的PowerShell脚本映射Active Directory
...
- **[攻击者利用疑似AI生成的PowerShell脚本映射Active Directory](https://www.freebuf.com/articles/ai-security/490119.html)**
  - 攻击者利用AI生成PowerShell脚本快速映射AD，72小时内完成云入侵，AI已成网络攻击倍增器。

- **[黑客利用Vibe-Coded生成的PowerShell脚本枚举Active Directory账户](https://www.freebuf.com/articles/ai-security/490127.html)**
  - AI生成“vibe-coded”PowerShell脚本绕过传统签名检测，AD枚举攻击行为难以隐藏，威胁已转向行为分析。

- **[从“提示词自律”到“代码层强制”-AI Skill开发中的自然语言约束失效](https://www.freebuf.com/articles/ai-security/489999.html)**
  - 为什么提示词永远无法真正约束AI的行为，我们应该如何在代码层建立真正的约束机制。

- **[AI大模型攻击思路，涵盖提示词劫持、知识库投毒、多语种混淆 + 身份伪装、诱导 AI 输出涉密后台核心配置](https://www.freebuf.com/articles/489948.html)**
  - 全套AI大模型实战攻击思路，覆盖提示词劫持、RAG知识库投毒、多语种混淆绕过、身份伪装诱导四大核心手法。

- **[AI agent 工具调用权限没隔离有多危险：一个 prompt 把你的文件偷走了](https://www.freebuf.com/articles/ai-security/489925.html)**
  - AI agent 工具调用权限没隔离有多危险：一个 prompt 把你的文件偷走了我上周在审一个内部 agent 平台的安全设计，翻到工具权限那块，愣住了。LangChain、AutoGen、CrewAI、Semantic Kernel，挨个看过去，权限模型全是同一个套路，全有或全无。agent 要么能调某个工具，要么不能，中间没有任何粒度。你给 agent 一个 file_read，一个 htt

- **[往向量库里塞三条文档就能控制 AI 的输出：RAG 投毒攻击实战拆解](https://www.freebuf.com/articles/ai-security/489924.html)**
  - RAG 投毒这个方向，2026 年之前基本停留在论文阶段，大家觉得"有道理但不太可能发生"。2026 年不一样了。

- **[成熟的AI该学会自己赚会员费了：一下午，它帮我薅了1000刀奖金](https://www.freebuf.com/articles/web/489905.html)**
  - 靠AI冲榜险胜却痛失榜首！人机协同破解2 Token极限反杀，揭露Agent幻觉与数学极限的博弈。
...
- **[DVWA SQL 注入实操笔记（Day2）— Boolean  Time 盲注 + SQLMap 一键脱库](https://www.freebuf.com/articles/web/490088.html)**
  - 本文按 DVWA 的 SQL Injection（Blind）模块走完 Low → Medium → High，然后引入 SQLMap 自动化注入，让你感受一下从手工&quot;猜&quot;到工具&quot;秒出&quot;的体验反差。

- **[一个双引号和一个美元符号撕开 CVSS 9.9 的 Nuclio（CVE-2026-52831）](https://www.freebuf.com/articles/vuls/490074.html)**
  - 一个双引号和一个美元符号撕开 CVSS 9.9 的 Nuclio（CVE-2026-52831）我翻了一遍 pkg/platform/kube/functionres/lazy.go第 2146 行附近那段代码，第一反应是这玩意儿不可能过 code review。一个 cron trigger 的 curl 参数，header key 直接往双引号里塞，body 用 strconv.Quote包一

- **[Wazuh（HIDS）从0-1上手实践](https://www.freebuf.com/articles/es/490049.html)**
  - 本文为博主自己学习wazuh的流程以及实践过程

- **[HTB Paperwork：源码泄露、PJL 路径穿越与 Unix Socket 提权](https://www.freebuf.com/articles/web/490007.html)**
  - Paperwork 主线：80 端口泄露 1515 源码，PJL 路径穿越写入 SSH 密钥拿到 `archivist`，再利用 SCM_RIGHTS 传 fd 读取 root 凭据提权。

- **[Nmap-对目标运行的操作系统和服务进行指纹识别](https://www.freebuf.com/articles/sectool/488452.html)**
  - 本教程将介绍如何利用 Nmap 对远程主机的操作系统和运行服务进行指纹识别。

- **[Mysql-JDBC反序列化利用与简单分析](https://www.freebuf.com/articles/vuls/489980.html)**
  - 渗透测试中利用JDBC反序列化漏洞，通过构造恶意MySQL协议包实现命令执行和文件读取，危害严重。

- **[PolinRider 朝鲜 108 包供应链攻击拆解 用区块链 RPC 当 C2](https://www.freebuf.com/articles/others-articles/489923.html)**
  - PolinRider 朝鲜 108 包供应链攻击拆解 用区块链 RPC 当 C2周一早上翻 The Hacker News，一条 108 个包的供应链新闻滑过去，第一反应是又来一波 typosquat

- **[2 小时被武器化：Adobe ColdFusion RDS FILEIO 路径穿越到未认证 RCE 全链拆解（CVE-2026-48282）](https://www.freebuf.com/articles/vuls/489922.html)**
  - 攻击者比厂商更早理解这套东西的危险性。别只盯着 0day，那些开了十年没人管的老功能才是真正的雷区。

- **[一个 .amazonqmcp.json 文件就能偷走你的 AWS 凭证：MCP 工作区信任边界设计缺陷拆解](https://www.freebuf.com/articles/ai-security/489921.html)**
  - 今天你 clone 的每一个仓库，在 MCP 客户端眼里都可能是一颗等待引爆的配置炸弹。

- **[NSA 恢复「TAO」番号的三重意图](https://www.freebuf.com/articles/489968.html)**
  - 美国NSA将精英黑客部门TAO恢复独立建制，逆转2016年拆分，意在提速网络攻击能力，加剧全球威胁。
