...
> 🔥 今日封面：AI-ML供应链攻击-从Pickle反序列化到模型命名空间劫持
...
- **[AI-ML供应链攻击-从Pickle反序列化到模型命名空间劫持](https://www.freebuf.com/articles/ai-security/491165.html)**
  - 1. 引言：AI供应链——信任即脆弱性AI/ML开发的本质决定了其对供应链的高度依赖。一个典型的ML项目不从头训练模型——它从Hugging Face下载预训练的BERT或Llama，通过pip安装transformers和datasets库，从公共S3存储桶拉取训练语料，并可能从GitHub克隆最新的微调脚本。每一个依赖项都是一个信任决策——而在当前的ML开发实践中，这些信任决策通常未经充分验证

- **[Claude Mythos 常见问题：能力、访问、竞争与影响](https://www.freebuf.com/articles/ai-security/491192.html)**
  - Claude Mythos加速网络威胁，AI工具将漏洞发现与利用时间压缩至极限，防御者必须转向持续漏洞运营。
...
- **[《AI Token中转站安全性评测报告》发布｜欢迎至 WAIC 2026 斗象展区领取](https://www.freebuf.com/consult/491038.html)**
  - 报告基于对国内 AI Token 中转站生态的持续调研与安全评测，从模型真实性、数据安全、响应完整性、账单透明度、服务稳定性五大维度，对主流 AI Token 中转站开展系统化分析与评估
...
- **[Mythos 并未击垮你的安全计划，真正危险的是你的暴露窗口](https://www.freebuf.com/articles/es/491263.html)**
  - Mythos没击垮你的安全计划，暴露窗口才是真威胁——攻击者29分钟突破，你的修复却需数周。

- **[Windows 绑定链接滥用：攻击者可致盲 EDR 并绕过 AMSI、AppLocker 和 Sysmon](https://www.freebuf.com/articles/system/491276.html)**
  - 攻击者利用Windows绑定链接可致盲EDR并绕过AMSI、AppLocker和Sysmon。

- **[严重 wp2shell RCE 漏洞：完整细节、PoC 及活跃利用情况](https://www.freebuf.com/articles/web/491277.html)**
  - 致命wp2shell漏洞链：无需认证即可实现预认证RCE，超5亿WordPress站点面临完全接管风险。

- **[HTTPS隧道里的银狐：一次DoH隐蔽通信木马的全链路分析](https://www.freebuf.com/articles/endpoint/491194.html)**
  - 银狐木马攻击团伙自2022年开始活跃，其传播手段包括：即时通讯工具钓鱼，邮件附件钓鱼，钓鱼网站，下载站冒充常用工具等方式。

- **[SQLi-Labs 实战：字符型 UNION 注入完整复现(Less-1,3,4)](https://www.freebuf.com/articles/vuls/491193.html)**
  - 本文是sql注入系列第二篇文章,详细介绍了字符型注入底层原理并手工复现.

- **[ServiceNow 关键沙箱 RCE 漏洞 PoC 公开](https://www.freebuf.com/articles/web/491220.html)**
  - PoC公开！ServiceNow沙箱RCE漏洞可致实例完全沦陷，攻击者无需认证即可执行代码。

- **[威胁行为者据称将星巴克数据发布至黑客论坛](https://www.freebuf.com/news/491221.html)**
  - 威胁行为者以400美元出售星巴克1.76亿条用户数据，包含卡详情与密码哈希，用户面临严重隐私与欺诈风险。

- **[15年历史的NGINX漏洞：攻击者可致Worker崩溃并实现远程代码执行](https://www.freebuf.com/articles/web/491177.html)**
  - 15年未修复的nginx漏洞可致堆溢出与RCE，影响所有版本，需紧急升级！

- **[NGINX 严重漏洞可导致工作进程崩溃，或允许远程代码执行](https://www.freebuf.com/articles/491150.html)**
  - NGINX严重漏洞可致远程代码执行，需立即升级至1.30.4或1.31.3。

- **[用激光重置硬件钱包密码？Tangem被Ledger Donjon攻破的技术内幕](https://www.freebuf.com/articles/vuls/491133.html)**
  - 攻击是真的，威胁是有限的，意识提升是长期的。
