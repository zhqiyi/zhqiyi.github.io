---
title: "安全日报 2026-08-14"
date: 2026-08-14 07:30:00 +0800
categories: 安全
tags: [安全, 网络安全, 行业, 日报]
...
> 🔥 今日封面：你打开仓库的那一刻，攻击就已经开始了 — AI 编码助手供应链暗战全解析
...
- **[你打开仓库的那一刻，攻击就已经开始了 — AI 编码助手供应链暗战全解析](https://www.freebuf.com/articles/ai-security/494766.html)**
  - 对于安全团队来说，唯一的出路是把 AI 编码助手的配置文件、凭据、依赖决策——全部纳入供应链安全的正式治理范围。

- **[AI 编码助手沙箱全是「纸糊的」？本地实测：Claude Code、Gemini CLI、Codex CLI 逃逸全记录](https://www.freebuf.com/articles/vuls/494760.html)**
  - 一条零权限 GitHub Issue 击穿三大 AI 编码助手沙箱，本地实测复现 CVSS 10.0 级漏洞与 HF 隐蔽外泄信道，附防御清单。

- **[LiteLLM 遭 40 分钟入侵，2,488 家公司的云密钥与 CI/CD 机密泄露](https://www.freebuf.com/articles/development/494817.html)**
  - 40分钟LiteLLM供应链攻击致2488家企业云密钥与CI/CD机密泄露，构建环境遭严重入侵。

- **[克隆一个仓库，拿下你的编码 Agent：Claude Code沙箱逃逸与Copilot自动批准翻转实战](https://www.freebuf.com/articles/ai-security/494685.html)**
  - 本文要回答的问题是:从一个被克隆的仓库出发,攻击者到底能走多远?

- **[「浏览器即攻击面」：AI 浏览器智能体的「零点击」攻击复现](https://www.freebuf.com/articles/web/494658.html)**
  - 本文拆解 2026 年海外四大实战研究（PerplexedBrowser / HashJack / AutoJack / BioShocking），并在零依赖本地环境完成全链路实战验证

- **[FreeBuf早报 \| API漏洞暴露大模型隐藏推理轨迹；Windows 0Day漏洞遭黑客利用](https://www.freebuf.com/news/494638.html)**
  - 重大缺陷！三大AI厂商API漏洞可跨模型重放加密推理块，明文泄露隐藏思维链及PII。

- **[从恶意网页到宿主机Shell：AutoJack三漏洞链如何让浏览器内嵌Agent成为RCE入口](https://www.freebuf.com/articles/ai-security/494625.html)**
  - 你只需要让 AI 浏览器 Agent 访问一个恶意网页——什么都不用点、什么都不用批——宿主机就会被完全拿下。

- **[TrustFall：两个 JSON 文件攻陷所有主流 AI 编码工具——Claude CodeCursorCopilot 全中招](https://www.freebuf.com/articles/vuls/494617.html)**
  - 两个JSON文件攻陷Claude Code/Cursor/Copilot/Gemini CLI。clone仓库→按Enter→RCE，Anthropic拒认漏洞。CI/CD零点击触发。附完整复现。

- **[低代码 AI 编排平台的安全模型为什么总塌：从 Flowise 到 Langflow 的设计缺陷谱系](https://www.freebuf.com/articles/vuls/494590.html)**
  - 这篇文章不逐个拆 Flowise 的 CVE，拿这批洞当锚点，讲低代码 AI 编排平台在安全模型设计上的系统性缺陷。
...
- **[渗透实战篇-拿到某个批发商的 OSS AccessKey 之后：从验证能读到证明高危泄露](https://www.freebuf.com/articles/endpoint/494678.html)**
  - 拿到密钥只是开始，能不能证明&quot;批量个人信息泄露&quot;才是漏洞成不成立的关键。整个链条走通，一个 AK 就是高危。
...
- **[Ransomware Threats in Europe H1 2026: A Deep Dive into Regional Attack Patterns and Dominant Threat Actors](https://cyble.com/blog/ransomware-threats-in-europe-h1-2026/)**
  - Europe faced a ransomware onslaught in the first half of 2026 that sets a troubling precedent for the remainder of the year. According to Cyble Research and Intelligence Labs (CRIL), the region experi
...
- **[帮我查一下天气背后的越权：Text-to-SQL 智能体注入攻击实战——从 ICSE 2025 到本地可复现的数据库越权链](https://www.freebuf.com/articles/ai-security/494698.html)**
  - 本文不依赖任何商业 API，用 Ollama + qwen2.5:7b + SQLite 在本地完整复现了直接注入与数据投毒两条越权链，并给出三道可落地的防御方案与量化对比数据。
...
- **[一个请求头拿下 Milvus 集群：认证绕过与官方没修完的洞](https://www.freebuf.com/articles/vuls/494226.html)**
  - Milvus 信任了一些不该被外部伪造的东西。

- **[XSS实战: XSS类型原理到绕过决策树(1篇打通xss-labs)](https://www.freebuf.com/articles/vuls/494586.html)**
  - 本文涵盖xss三种类型原理解析到xss-labs通关, 详细剖析了源码级别的绕过思路

- **[实战复现 ICML 2026 语义缓存碰撞：一条预埋回答，让无辜提问“强制平仓”](https://www.freebuf.com/articles/database/494531.html)**
  - 本文不抄论文，而是把它在本地完整跑通：同样的攻击链、真实的时延数据、可一键复现的代码。

- **[朝鲜Lazarus集团利用Windows 0Day漏洞发动“梦想工作”行动](https://www.freebuf.com/articles/system/494795.html)**
  - 朝鲜Lazarus借Windows零日漏洞发动梦想工作攻击，植入Troy后门并劫持合法服务器，严重威胁国防航天业。

- **[思科防火墙 0Day 漏洞遭在野利用，可致拒绝服务（CVE-2026-20349）](https://www.freebuf.com/articles/es/494731.html)**
  - 思科防火墙曝0Day漏洞遭在野利用，无需凭据可致设备重载，VPN中断，官方敦促立即热修复。

- **[MCP协议安全攻防实战：从Tool Poisoning到Rug Pull的全链路攻击与防御](https://www.freebuf.com/articles/ai-security/494654.html)**
  - 安全机制的缺失正使其成为攻击者的新目标。

- **[一句“再想想”就烧光你的GPU：ReasoningBomb与推理模型的算力黑洞](https://www.freebuf.com/news/494643.html)**
  - 但安全视角下，这套设计等于把算力消耗的开关交到了用户手里：用户输入一句话，模型就要按那句话“想”多久。

- **[PWN的底层原理与ROP艺术](https://www.freebuf.com/articles/vuls/494615.html)**
  - 本文深入探讨了Pwn的底层机制，详细梳理了x86/x64寄存器、汇编堆栈帧（push、pop、leave、ret）的执行流变化，同时系统剖析了ROP的核心原理与实战变种。

- **[从 validate-then-dial 到 DNS rebinding：SSRF 防护体系的三层架构设计——以 SeaweedFS CVE-2026-73080 为锚点](https://www.freebuf.com/articles/vuls/494591.html)**
  - 从 validate-then-dial 到 DNS rebinding：SSRF 防护体系的三层架构设计——以 SeaweedFS CVE-2026-73080 为锚点大多数 SSRF 防御代码长得差不多。拿到用户给的 URL，解析出 host，DNS 解析一下，遍历返回的 IP 地址，检查是不是内网段，不是就放行。看起来没毛病，对吧。我前两天翻 SeaweedFS 那个 CVE-2026-73
